...
export class AccountsController implements Controller{
  ...
  public initializeRoutes() {
    this.routerService.putRoute('/api'+this.path+'/normal', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
    this.router.post(this.path+'/normal',
                    validationMiddleware(accounts_schema),
                    this.newNormalAccount);
    this.routerService.putRoute('/api'+this.path+'/service', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
    this.router.post(this.path+'/service',
                      validationMiddleware(accounts_schema),
                      this.newServiceAccount);
                      this.router.get(this.path, this.getAll);
      this.routerService.putRoute('/api'+this.path+'/basicInfo/byAccountId/:accountId', RouteAuthEnum.NONE, RouteOtherAuthEnum.NONE).finally(() => {
        this.router.get(this.path+'/basicInfo/byAccountId/:accountId', this.getBasicAccountInfo);
        this.routerService.putRoute('/api'+this.path+'/profile-about/byAccountId/:accountId', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
          this.router.get(this.path+'/profile-about/byAccountId/:accountId', this.getAbout);
          this.routerService.putRoute('/api'+this.path+'/byAccountId/:accountId', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
            this.router.get(this.path+'/byAccountId/:accountId', this.findById);
            this.router.patch(this.path+'/byAccountId/:accountId', validationUpdateMiddleware(accounts_schema), this.update);
            this.routerService.putRoute('/api'+this.path+'/avatar/byAccountId/:accountId', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
              this.router.put(this.path+'/avatar/byAccountId/:accountId', validationUpdateMiddleware(accounts_schema), this.update);
              this.routerService.putRoute('/api'+this.path+'/DTO', RouteAuthEnum.NORMAL, RouteOtherAuthEnum.NONE).finally(() => {
